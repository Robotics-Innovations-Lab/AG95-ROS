...
void gripcommands(const dh_gripper_msgs::GripperState& msg)
{
  ros::NodeHandle m;
  ros::ServiceClient client_sub;

  float force = msg.target_force;
  float speed = 100;
  float position= msg.target_position;
	...
	while (ros::ok())
	{
		...
		msg_g_ctrl.position = position;
		msg_g_ctrl.force = force;
		msg_g_ctrl.speed = speed;
		_gripper_ctrl_pub.publish(msg_g_ctrl);
...
{
	ros::NodeHandle n;
	ros::Subscriber geom = n.subscribe("gripper_pose_1", 1000, gripcommands);
    	ros::spin();
}
